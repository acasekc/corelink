                    ...
                    <tr>
                        <td style="background: linear-gradient(135deg, #3b82f6 0%, #06b6d4 100%); padding: 40px; text-align: center;">
                            ...
                            <p style="margin: 10px 0 0; color: rgba(255,255,255,0.9); font-size: 16px;">
                                Anthropic API — {{ now()->subWeek()->format('M j') }} to {{ now()->format('M j, Y') }}
                            </p>
                        </td>
                    </tr>
                        ...
                        <td style="padding: 40px;">
                            ...
                            @else
                                <p style="color: #e2e8f0; font-size: 16px; line-height: 1.6; margin: 0 0 25px;">
                                    {{ $summaries->count() }} project{{ $summaries->count() === 1 ? '' : 's' }} with Anthropic API configured.
                                </p>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td style="text-align: right;">
                                                            <span style="display: inline-block; padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: 600;
                                                                @if($summary['status'] === 'active') background-color: rgba(34, 197, 94, 0.2); color: #22c55e;
                                                                @elseif($summary['status'] === 'grace') background-color: rgba(245, 158, 11, 0.2); color: #f59e0b;
                                                                @elseif($summary['status'] === 'disabled') background-color: rgba(239, 68, 68, 0.2); color: #ef4444;
                                                                @else background-color: rgba(148, 163, 184, 0.2); color: #94a3b8;
                                                                @endif
                                                            ">
                                                    ...
                                                    <tr>
                                                        <td>
                                                            <div style="background-color: #2a2a4e; border-radius: 4px; height: 8px; overflow: hidden;">
                                                                <div style="background-color: {{ $summary['usage_percent'] > 100 ? '#ef4444' : ($summary['usage_percent'] > 80 ? '#f59e0b' : '#22c55e') }}; height: 8px; width: {{ min($summary['usage_percent'], 100) }}%; border-radius: 4px;"></div>
                                                            </div>
                                                        </td>
                                                    </tr>
                                                </table>
                                                ...
                                                <table width="100%" cellpadding="0" cellspacing="0">
                                                    <tr>
                                                        <td style="color: #94a3b8; font-size: 13px;">
                                                            Usage: <span style="color: #ffffff; font-weight: 600;">${{ number_format($summary['usage_dollars'], 2) }}</span>
                                                            / ${{ number_format($summary['allowance'], 2) }}
                                                            <span style="color: #64748b;">({{ $summary['usage_percent'] }}%)</span>
                                                        </td>
                                                        <td style="text-align: right; color: #94a3b8; font-size: 13px;">
                                                            Grace: ${{ number_format($summary['grace_threshold'], 2) }}
                                                        </td>
                                                    </tr>
                                                    @if($summary['overage_dollars'] > 0)
                                                        <tr>
                                                            <td colspan="2" style="color: #f59e0b; font-size: 13px; padding-top: 6px;">
                                                                Overage: ${{ number_format($summary['overage_dollars'], 2) }}
                                                                (with {{ $summary['markup_percentage'] }}% markup: ${{ number_format($summary['overage_with_markup_dollars'], 2) }})
                                                            </td>
                                                        </tr>
                                                    @endif
                                ...
                                <table width="100%" cellpadding="0" cellspacing="0" style="border-top: 1px solid #2a2a4e; padding-top: 20px; margin-top: 10px;">
                                    <tr>
                                        <td style="text-align: center; padding: 10px;">
                                            ...
                                            <span style="color: #ffffff; font-size: 20px; font-weight: 700;">${{ number_format($summaries->sum('usage_dollars'), 2) }}</span>
                                        </td>
                                        <td style="text-align: center; padding: 10px;">
                                            <span style="color: #64748b; font-size: 12px; display: block;">Projects Active</span>
                                            <span style="color: #22c55e; font-size: 20px; font-weight: 700;">{{ $summaries->where('status', 'active')->count() }}</span>
                                        </td>
                                        <td style="text-align: center; padding: 10px;">
                                            <span style="color: #64748b; font-size: 12px; display: block;">Needing Attention</span>
                                            <span style="color: #f59e0b; font-size: 20px; font-weight: 700;">{{ $summaries->where('status', '!=', 'active')->count() }}</span>
                                        </td>
                                    </tr>
                                </table>
                            @endif
