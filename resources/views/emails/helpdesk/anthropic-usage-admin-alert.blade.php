                        ...
                        <td style="padding: 40px;">
                            @if($alertType === 'limit_reached')
                                <p style="color: #e2e8f0; font-size: 16px; line-height: 1.6; margin: 0 0 25px;">
                                    The API key for <strong>{{ $config->project->name ?? 'Unknown Project' }}</strong> has been automatically disabled after exceeding the grace threshold.
                                </p>
                            @else
                                <p style="color: #e2e8f0; font-size: 16px; line-height: 1.6; margin: 0 0 25px;">
                                    The project <strong>{{ $config->project->name ?? 'Unknown Project' }}</strong> has exceeded its included API allowance and entered the grace period.
                                </p>
                            @endif
                                        ...
                                        <table width="100%" cellpadding="0" cellspacing="0">
                                            <tr>
                                                <td style="padding-bottom: 15px;">
                                                    ...
                                                    <span style="color: {{ $alertType === 'limit_reached' ? '#ef4444' : '#f59e0b' }}; font-size: 22px; font-weight: 700;">${{ number_format($config->cycleUsageDollars(), 2) }}</span>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="padding-bottom: 15px; border-top: 1px solid #2a2a4e; padding-top: 15px;">
                                                    ...
                                                    <span style="color: #ffffff; font-size: 16px; font-weight: 600;">${{ number_format($config->included_allowance, 2) }}</span>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="padding-bottom: 15px; border-top: 1px solid #2a2a4e; padding-top: 15px;">
                                                    <span style="color: #94a3b8; font-size: 13px;">Grace Threshold</span><br>
                                                    <span style="color: #ffffff; font-size: 16px; font-weight: 600;">${{ number_format($config->grace_threshold, 2) }}</span>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="padding-bottom: 15px; border-top: 1px solid #2a2a4e; padding-top: 15px;">
                                                    <span style="color: #94a3b8; font-size: 13px;">Overage Mode</span><br>
                                                    <span style="color: #ffffff; font-size: 16px; font-weight: 600;">{{ ucfirst($config->overage_mode->value) }}</span>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="padding-bottom: 15px; border-top: 1px solid #2a2a4e; padding-top: 15px;">
                                                    <span style="color: #94a3b8; font-size: 13px;">Key Status</span><br>
                                                    <span style="color: {{ $alertType === 'limit_reached' ? '#ef4444' : '#f59e0b' }}; font-size: 16px; font-weight: 600;">{{ $config->key_status->label() }}</span>
                                                </td>
                                            </tr>
                                            @if($config->overageCents() > 0)
                                                <tr>
                                                    <td style="border-top: 1px solid #2a2a4e; padding-top: 15px;">
                                                        <span style="color: #94a3b8; font-size: 13px;">Total Overage (with {{ $config->markup_percentage }}% markup)</span><br>
                                                        <span style="color: {{ $alertType === 'limit_reached' ? '#ef4444' : '#f59e0b' }}; font-size: 16px; font-weight: 600;">${{ number_format($config->overageWithMarkupCents() / 100, 2) }}</span>
                                                    </td>
                                                </tr>
                                            @endif
                                        </table>
